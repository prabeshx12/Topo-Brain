...
We repeated the two-judge evaluation on a second, independent architecture (FR-U-Net
\cite{acs2025semisupervised}) trained on the same split, judging its synthesis with our co-trained
head and the independent segmenter (Table~\ref{tab:cross}). The result is two-sided and we report
both parts honestly. First---the positive finding---the circular inflation is \emph{not} mere
favouritism toward the co-trained generator: applied to FR-U-Net's synthesis, which is
out-of-distribution for it, the co-trained head still inflates apparent grey-matter fidelity from a
true (independent-judge) ratio of $0.04$ to $0.56$, a ${\sim}14\times$ inflation comparable to the
${\sim}14\times$ it applies to our own model ($0.10 \rightarrow 1.38$). A co-trained segmenter thus
hides over-smoothing even in an architecture it never saw; the effect generalises across methods.
Second---the honest null---this inflation did \emph{not} reverse the ranking of the two methods:
both judges rank our model above FR-U-Net (co-trained $1.38$ vs $0.56$; independent $0.10$ vs
$0.04$), because both methods are inflated by a similar factor. A rank reversal requires methods
whose true and co-trained orderings differ; with two methods we did not observe one, and we do not
claim it. The generalisation of the \emph{hiding} effect, not a ranking flip, is the cross-method
result.

\begin{table}[t]\centering
\caption{Two synthesis methods, judged by the co-trained head vs an independent segmenter
(GM synthetic/real $\beta_0$, higher = ``more preserved''). The co-trained head inflates BOTH
methods ${\sim}14\times$ over the independent judge; the ranking is preserved.}\label{tab:cross}
\begin{tabular}{lcc}
\toprule
Judge & ours & FR-U-Net \\
\midrule
Co-trained head (coupled)   & 1.38 & 0.56 \\
Independent GMM (uncoupled) & 0.10 & 0.04 \\
\bottomrule
\end{tabular}
\end{table}
