...
Table~\ref{tab:dose} reports the same grey-matter ratio as the segmenter is decoupled from the
generator. It decreases monotonically---joint $0.98 \rightarrow$ detached $0.41 \rightarrow$
independent $0.10$---so the segmenter's certification of fidelity is a dose-response in its coupling
to the generator. The continuous ratio is the effect; the binary reversal count (9/10 at joint,
2/10 at detached) drops precisely because decoupling already begins to reveal the defect.

\begin{table}[t]\centering
\caption{Grey-matter synthetic/real $\beta_0$ ratio (median, 10 subjects) vs.\ segmenter--generator
coupling. Monotonic: bias scales with coupling.}\label{tab:dose}
\begin{tabular}{lcc}
\toprule
Coupling & median ratio & reads as \\
\midrule
Joint (gradient-coupled)      & 0.98 & preserved \\
Detached (output-coupled)     & 0.41 & partially revealed \\
Independent (uncoupled)       & 0.10 & over-smoothed \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Isolating coupling from the generator.} The joint and detached arms use different
generators, so the dose-response could in principle reflect generator differences rather than
segmenter coupling. Two observations bound this: the independent judge rates both models' synthesis
as severely over-smoothed ($0.10$ vs.\ $0.04$), while the \emph{co-trained} verdict swings far more
($0.98$ vs.\ $0.41$) than the underlying quality does. \textbf{[PENDING]} a confound-free variant
applies all three segmenters to the \emph{same} images, isolating segmenter coupling entirely.
